...
object FellowPlayerRecord {
  ...
  lazy val selectByPlayerId =
    s"""
      SELECT 
        pl.${PlayerSchema.playerId}, 
        pl.${PlayerSchema.playerName},
        t.${TeamSchema.teamId}, 
      ...
      FROM ${PlayerSchema.tableName} AS pl 
        ...
        INNER JOIN (
          ...
          INNER JOIN ${PlayerSchema.tableName} AS pl2 ON tp2.${TeamPlayerSchema.playerId} = pl2.${PlayerSchema.playerId} AND p2.${PlayerSchema.playerId}
          LEFT OUTER JOIN ${PlayerProfileSchema.tableName} AS pp ON pl2.${PlayerSchema.playerId} = ${PlayerProfileSchema.playerId} AND ${PlayerProfileSchema.isApproved} = 1
